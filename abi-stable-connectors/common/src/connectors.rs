//! In the connectors plugin interface there is a single `new` function that
//! exports a `Connector` dynamic trait from the plugin. The runtime can then
//! use that as a generic connector just like how Tremor does now. On the
...
//! on the plugin. What I mean is that, instead of calling `builder.spawn` on
//! the plugin and creating the channel on the plugin, it should happen on the
...
//!   has fields to make sure `Source` or `Sink` are properly implemented. All
//!   of the connectors implement `Source` and `Sink` always, but we can make it
//!   optional by adding a marker or something like that.
//!
//!   Spoiler: that won't work with just `abi_stable` anyway. Only with `cglue`,
//!   which makes it possible to have groups of traits. So it *would* be
